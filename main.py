...
def train(args):
    ...
    for epoch in range(args.epoch):
        train_loss = engine.train(train_loader, model, softmax, ce, mse, optimizer, scaler, use_amp, alpha, beta)
        validation_loss = engine.validation(validation_loader, model, softmax, ce, mse, alpha, beta)
...
def test(args):
    test_loader = data_loader.get_data_loader(args.validation_csv, args.img_dir, False, args.batch_size)
    ...
    model.load_state_dict(torch.load(args.test_model)) # Loading Trained Model
    model.to(device)
    softmax = nn.Softmax().to(device)

    engine.test(test_loader, model, softmax)

def main(args):
    if args.phase == "train": # Train
        train(args)

    elif args.phase == "test": # Test
        test(args)
...
if __name__=='__main__':
    ...
    parser.add_argument('--test_model', required=False, default=None, type=str)
